...
如果你部署了配套的后端 API 服务，可以选择此方式将数据持久化到本地 SQLite 数据库。

### 前提条件

已部署后端服务（参见 README 中的 Docker 部署章节），并已通过 `/admin/` 管理后台配置好 API 令牌。

### 填写到 Clean Nav 设置中

| Clean Nav 设置项 | 说明 |
| :--- | :--- |
| **后端地址** | 后端服务的地址，例如 `http://192.168.1.100:8642`（如果使用合体镜像，填写页面本身的地址即可） |
| **API 令牌** | 在管理后台 `/admin/` 获取的令牌（`sk-xxx`） |

**合体镜像用户**：前端和后端同源，自动免令牌认证，无需填写 API 令牌。
