...
## ☁️ 极速上手 (无需部署)
如果你不想购买服务器或折腾 Docker，你可以直接使用我的演示站，并将数据存储在你自己的 GitHub 私有仓库或 Cloudflare R2 中。
...
- [点击查看 GitHub 配置指南](./README.md#2-获取访问令牌-access-token) (见下方)

### 2. S3 / Cloudflare R2 (推荐)
支持 AWS S3、Cloudflare R2 等兼容 S3 协议的对象存储。R2 提供 10GB 免费空间，速度快且稳定。
- [📘 点击查看 Cloudflare R2 / S3 配置教程](./docs/storage-guide.md)

---

## ☁️ GitHub 同步配置流程 (Legacy)
### 1. 创建数据仓库 (存放数据)
