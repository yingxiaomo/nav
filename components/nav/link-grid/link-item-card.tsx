...
export function LinkItemCard({ item, onClick, className }: LinkItemCardProps) {
    ...
    return (
       <div className={`group block relative ${className || ''}`}>
           <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/10 transition-colors">
               <div className={`h-8 w-8 shrink-0 rounded-lg flex items-center justify-center border border-white/20 overflow-hidden bg-${isFolder ? 'yellow' : 'blue'}-500/10 text-${isFolder ? 'yellow' : 'blue'}-500`}>
                   <IconRender name={item.icon || (isFolder ? "FolderOpen" : "Link")} className="h-4 w-4" />
               </div>
               <div className="min-w-0 flex-1">
                   <h4 className="text-white font-medium text-sm" title={item.title}>
                       {item.title}
               ...
               {isFolder && <ChevronLeft className="h-4 w-4 text-white/50 rotate-180" />}
           </div>
       </div>
    );
}